...
def load_frf_dataset_csv(
    ...
    """
    Loads complex VELOCITY FRF data from CSV.
    Returns: (omega, x, Vr, Vi) all shaped (N,1)
    """
    ...
    Vr = torch.tensor(df[real_col].to_numpy(), dtype=torch.float32, device=device).view(-1, 1)
    Vi = torch.tensor(df[imag_col].to_numpy(), dtype=torch.float32, device=device).view(-1, 1)

    return (omega, x, Vr, Vi)
